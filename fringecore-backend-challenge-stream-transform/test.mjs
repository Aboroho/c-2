...
function check() {
  if (
    ...
    console.log(buffer);
    console.log(
      'OH NO :( -- the output should not contain "i like big trains and i cant lie", but it seems it still has it'
    );
    process.exit(0);
  }
}
...
setInterval(() => {
  ...
  if (lastIncreaseTime < Date.now() - 1000) {
    console.log(
      "OH NO :( -- the output should increase every second or so, but it does not seem to be doing that."
    );
    // process.exit(0);
  }
